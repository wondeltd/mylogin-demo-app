<x-guest-layout>
    <div class="text-center p-4" x-data="{ protocol: 'oauth', authTarget: '' }" >
        ...
        <form @submit.prevent="submitProtocol(authTarget, protocol)" class="text-center">
    ...
    <script>
        async function submitProtocol(authTarget, protocol)
        {
            if (!authTarget) {
                alert('Please select an authentication target');
                return;
            }
            window.location.href = "/" + authTarget + "/redirect?protocol=" + protocol;
        }
    </script>
</x-guest-layout>
